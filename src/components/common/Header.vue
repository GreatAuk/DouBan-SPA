<template>
	<header>
		<router-link to="/">
			<h1 class="logo">豆瓣</h1>
		</router-link>
		<nav>
			<ul>
				<li>
					<router-link to="/movie" class="nav-movie">电影</router-link>
				</li>
				<li>
					<router-link to="/book" class="nav-book">图书</router-link>
				</li>
				<li>
					<router-link to="/radio" class="nav-radio">广播</router-link>
				</li>
				<li>
					<router-link to="/group" class="nav-group">小组</router-link>
				</li>
				<li>
					<router-link to="/search" class="nav-search"></router-link>
				</li>
			</ul>
		</nav>
	</header>
</template>

<script>
export default {

}
</script>

<style lang="scss" scoped>
@import '../../assets/scss/helper.scss';

header {
	padding: 0 px2rem(36);
	position: fixed;
	left: 0;
	right: 0;
	top: 0;
	z-index: 10;
	background: #fff;
	height: px2rem(47*2);
	display: flex;
	align-items: center;
	justify-content: space-between;
	border-bottom: px2rem(2) solid #f3f3f3;
	.logo {
		height: px2rem(22*2);
		width: px2rem(46*2);
		font-size: 0;
		background-size: cover;
		background-image: url('data:image/png;base64,');
	}
	nav {
		li {
			display: inline-block;
			@include font-dpr(30);
			vertical-align: middle;
			&:last-child {
				margin-right: 0;
			}
			a {
				padding: px2rem(16);
			}
		}
		.nav-movie {
			color: #2384E8;
		}
		.nav-book {
			color: #9F7860;
		}
		.nav-group {
			color: #2AB8CC;
		}
		.na-radio {
			color: #E4A813;
		}
		.nav-search {
			display: inline-block;
			width: px2rem(50);
			height: px2rem(36);
			margin-top: px2rem(10);
			background-repeat: no-repeat;
			background-image: url('data:image/png;base64,');
			background-size: cover;
		}
	}
}
</style>
